...
export async function fetchMinisite(slug: string): Promise<MinisiteData | null> {
  if (!STORYPAY_URL) return null;
  try {
    const res = await fetch(`${STORYPAY_URL}/api/public/minisite/${encodeURIComponent(slug)}`, {
      cache: "no-store",
    });
    if (!res.ok) return null;
    return (await res.json()) as MinisiteData;
  } catch {
    return null;
  }
}
