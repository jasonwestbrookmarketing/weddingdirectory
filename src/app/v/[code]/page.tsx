...
// StoryPay backend (app.storyvenue.com) owns the resolver + tracking endpoints
// and reads with the service role, so no anon grant is needed here — this page
// only calls those APIs over HTTP.
const APP_BASE = (
...
async function recordScan(venueId: string, code: string) {
  ...
  after(async () => {
    try {
      await fetch(`${APP_BASE}/api/listing-track`, {
        method: "POST",
        headers: {
...
export default async function ShortLinkPage({
  ...
  try {
    const res = await fetch(
      `${APP_BASE}/api/public/lead-link/${encodeURIComponent(handle)}`,
      {
        next: { revalidate: 0 },
