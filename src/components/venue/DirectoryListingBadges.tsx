/**
 * Instagram-style verified check + "Sponsored" pill for published venues.
 * Style-matched with the StoryPay dashboard version so the same listing
 * looks identical across app.storyvenue.com and storyvenue.com.
 */
export function DirectoryListingBadges({
  ...
  verified: boolean;
  sponsored: boolean;
  /** Pick the pill style that sits best on top of your background. */
  variant?: "onDark" | "onLight";
  /** md = 22px check (listing hero), sm = 16px check (cards). */
  size?: "sm" | "md";
  /** Extra classes (e.g. margin) so callers don't fight the defaults. */
  className?: string;
}) {
  if (!verified && !sponsored) return null;

  const checkSize = size === "sm" ? 16 : 22;
    ...
    >
      {verified ? (
        ...
        >
          ...
          >
            {/*
              Meta-style 12-point rosette: 24 vertices alternating between
              outer radius 11.5 and inner radius 9.8 at 15° increments, so
              the outline reads as 12 pronounced points. Matches the blue
              check couples recognize from Instagram / Facebook.
            */}
            <path
