...
export default function Guestbook({ slug }: { slug: string }) {
  ...
  return (
    <section className="mt-10">
      ...
      <form onSubmit={submit} className="mx-auto mt-4 max-w-[520px] rounded-2xl border border-brand-line bg-white p-4 shadow-[0_10px_30px_-20px_rgba(0,0,0,0.35)]">
        <input
          value={name}
      ...
      {entries.length > 0 && (
        <div className="mx-auto mt-5 max-w-[520px] space-y-3">
          {entries.map((entry) => (
            <div key={entry.id} className="rounded-2xl border border-brand-line bg-white px-4 py-3 shadow-[0_10px_30px_-24px_rgba(0,0,0,0.3)]">
